...
export const BugHolderScreen = () => {
  ...
  const startWanderLoop = (bug: BugData, animation: BugAnimation) => {
    ...
    const run = () => {
      ...
      const stepX = randomBetween(-120, 120);
      const stepY = randomBetween(-120, 120);
      const targetX = Math.min(
        bounds.maxX,
        Math.max(bounds.minX, currentX + stepX)
      );
      const targetY = Math.min(
        bounds.maxY,
        Math.max(bounds.minY, currentY + stepY)
      );
      ...
      Animated.parallel([
        ...
        const pause =
          Math.random() < 0.2 ? randomBetween(250, 700) : 0;
  ...
  const startJitterLoop = (bug: BugData, animation: BugAnimation) => {
    ...
    const run = () => {
      ...
      Animated.parallel([
        ...
        loopState.jitterTimeout = setTimeout(run, randomBetween(200, 400));
      });
    };
  ...
  return (
    ...
    >
      ...
      {bugs.map((bug) => {
        ...
        const eyeSize = Math.round(size * 0.15);
            ...
            >
              <View
                style={[
                  styles.bugBody,
                  {
                    backgroundColor: getDifficultyColor(bug.difficulty),
                    borderColor: getDifficultyBorderColor(bug.difficulty),
                    borderRadius: size / 2
                  }
                ]}
              >
