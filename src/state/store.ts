...
const initialState = {
  todos: [] as Todo[],
  recurrent: [] as Recurrent[],
  coins: 0,
  infestation: 0,
  monthlyStats: {} as MonthlyStatsMap,
  loading: true,
  recentlyDeleted: null as BugBiteState["recentlyDeleted"],
  lastAction: null as BugBiteState["lastAction"]
};
...
export const useBugBiteStore = create<BugBiteState>()(
  persist(
    (set, get) => ({
      ...
      resetAllData: async () => {
        await AsyncStorage.removeItem("bugbite-store");
        set({
          ...initialState,
          loading: false
        });
